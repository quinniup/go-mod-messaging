...
func (cc ClientConfig) ConnectOpt() ([]nats.Option, error) {
	...
	opts := []nats.Option{
		...
		nats.DisconnectErrHandler(func(nc *nats.Conn, err error) {
			if err != nil {
				pkg.Log.Warnf("NATS Client Disconnected due to: %s, will attempt reconnect\n", err)
			}
		}),
		nats.ReconnectHandler(func(nc *nats.Conn) {
			pkg.Log.Infof("NATS Client Reconnected [%s]\n", nc.ConnectedUrl())
		}),
		nats.ErrorHandler(func(nc *nats.Conn, _ *nats.Subscription, err error) {
			url := nc.ConnectedUrl()
			if url == "" {
				pkg.Log.Errorf("NATS Client Unexpected error: %s", err)
			} else {
				pkg.Log.Errorf("NATS Client Unexpected error from server %s: %s", url, err)
			}
		}),
	}
